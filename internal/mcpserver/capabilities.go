...
type ScriptingAPIInfo struct {
	...
	NetWrappers    []string           `json:"net_wrappers"`
...
func Capabilities() CapabilityDocument {
	return CapabilityDocument{
		...
		ClientBootstrap: ClientBootstrapInfo{
			...
			SafeEvalProbe:         `const caps = sys.capabilities(); return {name: caps.name, wrappers: caps.scripting_api.uapi_wrappers, os: caps.scripting_api.os_wrappers, io: caps.scripting_api.io_wrappers, net: caps.scripting_api.net_wrappers, examples: caps.example_scripts, uname: sys.uname(), openFlags: sys.constants({group:"open_flags"}).constants};`,
			EvalToolExample: map[string]any{
				"name": "eval",
				"arguments": map[string]any{
					"script": `const caps = sys.capabilities(); return {name: caps.name, sys: caps.scripting_api.uapi_wrappers.length, os: caps.scripting_api.os_wrappers.length, io: caps.scripting_api.io_wrappers.length, net: caps.scripting_api.net_wrappers.length, uname: sys.uname()};`,
				},
			},
			...
			Notes: []string{
				...
				"The JavaScript os, io, and net globals provide Go standard-library style wrappers over managed handles and explicit byte encodings; they are not Node.js modules.",
		...
		PrimitiveGroups: []PrimitiveGroup{
			...
			{"Go standard library scripting", "Use package-os, package-io, and package-net style helpers for portable filesystem, environment, root, process, stream, socket, packet, and DNS workflows over managed handles.", []string{"os.readFile", "os.writeFile", "os.open", "os.openFile", "os.openRoot", "os.rootReadFile", "os.fileRead", "os.fileWrite", "io.copy", "io.copyN", "io.readAll", "io.readFull", "io.writeString", "net.dial", "net.listen", "net.listenPacket", "net.lookupHost", "net.connRead", "net.packetReadFrom"}},
			{"JavaScript scripting", "Compose multi-step syscall and standard-library workflows in one eval call with captured logs, args, timeouts, and JSON results.", []string{"eval", "sys.*", "uapi.*", "os.*", "io.*", "net.*"}},
...
func scriptingAPIInfo() ScriptingAPIInfo {
	return ScriptingAPIInfo{
		...
		Globals:        []ScriptGlobalInfo{{"args", "Caller-provided JSON object."}, {"console", "Captured log/info/warn/error functions returned in the eval response."}, {"print", "Alias for console.log."}, {"sys", "Synchronous Linux sys/unix and self-contained eBPF scripting API with managed FD, buffer, mmap, socket, process, xattr, event, map, program, link, and reader helpers."}, {"uapi", "Alias for sys for compatibility with older scripts."}, {"os", "Synchronous Go package os style wrappers for files, dirs, env, roots, processes, and File methods over managed handles."}, {"io", "Synchronous Go package io style wrappers for copy/read/write helpers over managed handle, net connection, buffer, path, data, and discard endpoints."}, {"net", "Synchronous Go package net style wrappers for TCP/IP, UDP, Unix domain sockets, DNS resolution, interfaces, and managed net.Conn/net.Listener/net.PacketConn handles."}},
		...
		Conventions:    []string{"Call MCP resources/read outside eval for uapi://agent-guide, uapi://scripting-api, uapi://api-reference, uapi://api, and uapi://examples.", "Call the MCP tool named eval with an object containing script, optional args, timeout_ms, max_log_entries, and max_result_bytes.", "Read API-specific docs from uapi://api/<name> and reusable scripts from uapi://examples/<file>.js before composing a subsystem-specific workflow.", "Inside scripts, use sys.* or uapi.* for Linux syscalls and eBPF helpers, os.* for Go package os style workflows, io.* for Go package io style copy/read/write workflows, and net.* for Go package net style TCP/UDP/Unix/DNS workflows.", "eBPF program loading accepts built-in kinds such as return, counter, perf_event, ringbuf_event, socket_filter_pass, and socket_filter_drop; scripts do not need target-side C compilation, raw assembly, or ELF loading.", "Every syscall-style helper returns ok=true on success or ok=false with errno fields for Linux errno failures; os/io/net package errors return ok=false with error_name/error_type/path/network/timeout metadata when available.", "Ptrace helpers pin the eval goroutine to one OS thread from the first ptrace wrapper call through script completion.", "sys.ptraceGetRegs returns normalized syscall metadata with a generated name when the syscall number is known for the architecture.", "Close managed FDs, net connections, net listeners, net packet connections, roots, mappings, eBPF links, eBPF programs, eBPF maps, and eBPF readers explicitly when a script creates them."},
		UAPIWrappers:   scriptWrapperNames(),
		OSWrappers:     scriptOSWrapperNames(),
		IOWrappers:     scriptIOWrapperNames(),
		NetWrappers:    scriptNetWrapperNames(),
		...
		Examples: []ScriptingExample{
			{Name: "safe_bootstrap", Script: `const caps = sys.capabilities(); return {name: caps.name, sys: caps.scripting_api.uapi_wrappers, os: caps.scripting_api.os_wrappers, io: caps.scripting_api.io_wrappers, net: caps.scripting_api.net_wrappers, examples: caps.example_scripts, uname: sys.uname()};`},
			...
			{Name: "net_tcp_echo", Script: `const listener = net.listen({network:"tcp", address:"127.0.0.1:0"}); const addr = net.listenerAddr({listener: listener.listener}).addr.address; const client = net.dial({network:"tcp", address: addr, timeout_ms:1000}); const server = net.listenerAccept({listener: listener.listener, timeout_ms:1000}); net.connWrite({conn: client.conn, data_utf8:"ping"}); const got = net.connRead({conn: server.conn, length:4, encoding:"utf8", timeout_ms:1000}); net.connClose({conn: client.conn}); net.connClose({conn: server.conn}); net.listenerClose({listener: listener.listener}); return got;`},

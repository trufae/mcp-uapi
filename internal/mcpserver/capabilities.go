...
func scriptingAPIInfo() ScriptingAPIInfo {
	return ScriptingAPIInfo{
		...
		Conventions:    []string{"Call MCP resources/read outside eval for uapi://agent-guide, uapi://scripting-api, uapi://api-reference, uapi://api, and uapi://examples.", "Call the MCP tool named eval with an object containing script, optional args, timeout_ms, max_log_entries, and max_result_bytes.", "Read API-specific docs from uapi://api/<name> and reusable scripts from uapi://examples/<file>.js before composing a subsystem-specific workflow.", "Inside scripts, use sys.* or uapi.* for Linux syscalls, os.* for Go package os style workflows, and io.* for Go package io style copy/read/write workflows.", "Every syscall-style helper returns ok=true on success or ok=false with errno fields for Linux errno failures; os/io package errors return ok=false with error_name/error_type/path metadata when available.", "Ptrace helpers pin the eval goroutine to one OS thread from the first ptrace wrapper call through script completion.", "sys.ptraceGetRegs returns normalized syscall metadata with a generated name when the syscall number is known for the architecture.", "Close managed FDs, roots, and mappings explicitly with sys.close or os.fileClose, os.rootClose, and sys.munmap when a script creates them."},

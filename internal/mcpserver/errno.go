...
func errnoNames() map[unix.Errno]string {
	names := map[unix.Errno]string{
		...
		unix.ESTALE: "ESTALE", unix.EDQUOT: "EDQUOT", unix.ECANCELED: "ECANCELED", unix.EOWNERDEAD: "EOWNERDEAD", unix.ENOTRECOVERABLE: "ENOTRECOVERABLE",
	}
	for errno, name := range platformErrnoNames() {
		names[errno] = name
	}
	return names
}
